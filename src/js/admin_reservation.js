...
function constructRoom() {
    ...
    document.getElementById("reserve-btn").addEventListener("click",displayModal);
...
function inpersonReserve() {
    ...
    for (let i = 0; i < selected_seats.length; i++)
    {
        ...
        for (let j = 0; j < reservations.length; j++)
            if (isOccupied(reservation,reservations[j]))
                available = false;
    }
...
function removeReservation(event)
{
    ...
    selected_room.reservations[parseInt(event.currentTarget.id.slice(7,8))][parseInt(event.currentTarget.id.slice(9,10))].splice(parseInt(event.currentTarget.id.slice(11,12)));
