export function ChangePassword() {
    return (
        <div className="m-5 flex gap-4 align-center justify-center">
            <div className="rounded-2xl pb-2 gray-67 flex flex-col items-center w-9/12">
                <div className="w-full">
                    <div className="m-7">
                        <div className="text-2xl mb-6 google font-bold mt-4">
                            Change Password
                        </div>
                        Change the password of your account
                    </div>
                    
                </div>
                <div className="flex flex-col items-center justify-center gap-[10px]">
                    <form> 
                        <table className="border-separate border-spacing-0">
                            <tbody>
                                <tr>
                                    <th align="left" className="py-3 font-bold">Old Password</th>
                                    <td className="w-[150px] py-3">
                                        <input type="password" className="w-full rounded border border-gray-400 p-1 focus:outline-none focus:ring-2 focus:ring-[#006341]" />
                                    </td>
                                </tr>

                                <tr>
                                    <th align="left" className="py-3 font-bold">New Password</th>
                                    <td className="w-[150px] py-3">
                                        <input type="password" className="w-full rounded border border-gray-400 p-1 focus:outline-none focus:ring-2 focus:ring-[#006341]" />
                                    </td>
                                </tr>

                                <tr>
                                    <th align="left" className="py-3 font-bold">Confirm Password</th>
                                    <td className="w-[150px] py-3">
                                        <input type="password" className="bg-gray-89 confirm-password w-full rounded border border-gray-400 p-1 focus:outline-none focus:ring-2 focus:ring-[#006341]" />
                                    </td>
                                </tr>

                                <tr>
                                    <th colSpan="2" align="right" className="pt-4">
                                        <button type="submit" 
                                            className="mt-[3px] px-[6px] py-[3px] bg-[#C5C5C5] text-black
                                            rounded-md flex justify-center items-center
                                            active:scale-95 transition-transform transform hover:scale-103">Submit</button>
                                    </th>
                                </tr>
                            </tbody>
                        </table>
                    </form>
                </div>
            </div>
        </div>
    )
}